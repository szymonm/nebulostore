...
do
    ...
    if [ $i -eq $PEER_NUM ]
    then
        CLASS_NAME=$TESTPEERNAME
    else
        CLASS_NAME=$PEERNAME
    fi

    padded=`printf "%02d" $i`
    ./resources/conf/generate_config.py $COMMON_ARGS --APP_KEY=$i$i --CLASS_NAME=$CLASS_NAME --TEST_LIST=$CONCAT\
         --BOOTSTRAP_MODE=$BOOTSTRAP_MODE --CLI_PORT=101$padded --TOMP2P_PORT=103$padded --BDB_TYPE=$BDB_TYPE\
         --BOOTSTRAP_ADDRESS=$BOOTSTRAP_ADDRESS < ./resources/conf/Peer.xml.template > Peer.xml.$i
done
