...
public class HashCalculator {
  ...
  /**
   * Calculates bundled hashes of the set of keys which is the intersection of
   * keys that both the local node and the specified node are responsible for.
   * The keys are bundled in bundles of exponentially increasing size in the
   * time difference from <code>time</code>, so that the older the timestamp the
   * larger the bundle. Keys with timestamp greater then <code>time</code> are
   * grouped in one large bundle. The returned list will contain <i>O(log N)</i>
   * hashes, where <i>N</i> is the number of mappings.
   * 
  ...
  /**
   * Removes TimestampedEntry objects from the end of <code>mappings</code>
   * until a mapping is encountered with timestamp less than <code>time</code>.
   * The mappings removed are returned in a List in the order that they were
   * removed. That is, the reverse order of the ordering in
   * <code>mappings</code>.
   **/
  private List tailFrom(List mappings, long time) {
    List tail = new ArrayList();
    while (mappings.size() > 0) {
      TimestampedEntry entry = (TimestampedEntry) mappings
          .get(mappings.size() - 1);
   ...
   * <code>begin</code> (including) and <code>end</code> (not including) that
   * the specified node is also responsible for. That is, the specified node and
   * the local node is one of the <i>K</i> closest nodes to each key. The
   * mappings are returned as {@link TimestampedEntry} objects in a List sorted
   * by their timestamp in ascending order.
   **/
  ...
  /**
   * Calculates a unique hash of the keys in the specified bundle. The bundle is
   * a list of TimestampedEntry objects. To get consistent hashing the bundle
   * should be ordered in a deterministic way. This is that case for lists
   * returned by {@link #mappingsBetween}.
   **/
  public byte[] calculateHash(List bundle) {
    try {
      ...
      if (md.getDigestLength() != HASH_LENGTH) {
        throw new RuntimeException("Unexpected hash length " +
            md.getDigestLength() + " - expected " + HASH_LENGTH);
      }
