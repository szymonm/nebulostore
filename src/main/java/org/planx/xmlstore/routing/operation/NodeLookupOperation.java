...
/**
 * Finds the <i>K</i> closest nodes to a specified identifier. The algorithm
 * terminates when the operation has gotten responses from the <i>K</i> closest
 * nodes it has seen. Nodes that fail to respond are removed from consideration.
 **/
public class NodeLookupOperation extends Operation implements Receiver {
  ...
  /**
   * Message sent to each peer. Subclasses could change this field to send other
  ...
  /**
   * Keeps track of which nodes have been asked and gotten reply from. Nodes are
   * keys mapped to a Byte indicating the status. The map is kept sorted
   * according to closeness to the sought after identifier.
   **/
  private final SortedMap nodes;

  /**
   * Keeps track of messages in transit and awaiting reply. No more than
   * <code>CONCURRENCY</code> message may be in transit and unreplied at the
   * same time. Communication ids as Integers are keys and Nodes are values.
   **/
  private final Map transit;
  ...
  /**
   * @return A List containing the <i>K</i> closest nodes to the id provided in
   *         the constructor.
   ...
   * and a thread waiting on this object is woken up. Returns <code>true</code>
   * if finished and <code>false</code> otherwise.
   **/
  private boolean askNodesOrFinish() throws IOException {
    if (transit.size() >= conf.CONCURRENCY)
      return false;
    ...
    for (int i = 0; (transit.size() < conf.CONCURRENCY) && (i < ask.size()); i++) {
      ...
      int comm = server.send(lookupMessage, node.getAddress(), this);
  ...
  /**
   * Returns the subset of nodes with the specified status among the K nodes
   * encountered closest to the target that have status not FAILED.
   **/
