...
public class QueryThread implements Runnable {
  ...
  static private final long ackTimeout_ = 500;
  static private final long queryTimeout_ = 1000;
  ...
  public void run() {
    ...
    try {
      ...
      if (currentDepth_ < maxDepth_) {
        System.out.println("\t\tQuery depth (" + currentDepth_ + "/" +
            maxDepth_ + "). Forwarding...");
        System.out.println("\t\tRunning forward...");
        state = interpreter_.runForward(preparedQuery, state);

        System.out.println("\tFORWARD result: " + state.getPeersToForward());
        ...
        for (IDQLValue appKeyDQLValue : peersToForward) {
          ...
          System.out.println("\t\t Forwarding query to: " + peerAddress);
          ...
          timeElapsed_.put(peerAddress, 0L);
        }
            ...
            if (message != null) {
              ...
              if (queryState_.get(commMessage.getSourceAddress()) != RemoteQueryState.Finished) {
                ...
                if (message instanceof QueryAcceptedMessage) {
                  logger_.debug("Got QueryAcceptedMessage from: " +
                      commMessage.getSourceAddress().toString());
                ...
                if (message instanceof QueryResultsMessage) {
                  ...
                  System.out.println("\t Got result from: " +
                      resultsMessage.getSourceAddress() + "\t:\t" + result);
      ...
      // moving to reduce...
      System.out.println("\t\tComputing reduce...");
      state = interpreter_.runReduce(preparedQuery, state);
      System.out.println("\tREDUCE results: " + state.getReduceResult());

      logger_.info("Sending back results of the query");

      dqlExecutor_.sendQueryResults(new QueryResultsMessage(senderJobId_, null,
          sender_, queryId_, state.getReduceResult()));
  ...
  private void updateTimeouts(long delay) {
    ...
    logger_.debug("updateTimeouts called. queryState_: " +
        queryState_.toString());
    logger_.debug("updateTimeouts called. timeElapsed_: " +
        timeElapsed_.toString());
