...
public abstract class PrivacyLevel implements Serializable {
  ...
  public PrivacyLevel generalize(PrivacyLevel l, IDQLValue first,
      IDQLValue second, IDQLValue result, boolean isNeutralGenerator) throws InterpreterException {
    if (isMorePublicThan(l))
      return l.freshCopy().performGeneralize(this, first, second, result, isNeutralGenerator)
          .mergeSources(this);
    if (isLessPublicThan(l))
      return this.freshCopy().performGeneralize(l, first, second, result, isNeutralGenerator)
          .mergeSources(l);
  ...
  protected abstract PrivacyLevel performGeneralize(PrivacyLevel l,
      IDQLValue first, IDQLValue second, IDQLValue result, boolean isNeutralGenerator)
          throws InterpreterException;
