 ...
 * mappings in the requested interval that should be available at the origin
 * node.
 **/
public class StoreRequestReceiver extends OriginReceiver {
  ...
  public void receive(KademliaInternalMessage incoming, int comm)
    ...
    List mappings = hasher.mappingsBetween(origin, mess.getBegin(),
        mess.getEnd());
    for (int i = 0, max = mappings.size(); i < max; i++) {
      TimestampedEntry entry = (TimestampedEntry) mappings.get(i);
      KademliaInternalMessage rep = new StoreMessage(local, entry.getKey(),
          entry.getValue(), true);
      server.send(rep, origin.getAddress(), null);
    }
  }
}
