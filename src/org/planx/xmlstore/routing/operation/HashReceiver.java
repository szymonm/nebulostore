...
/**
 * Receives a HashMessage and compares it to hashes generated locally. Any hash
 * for an interval not matching will result in a StoreRequestMessage for that
 * interval.
 **/
public class HashReceiver extends OriginReceiver {
  ...
  public HashReceiver(MessageServer server, Node local, Space space,
      Map localMap) {
  ...
  public void receive(KademliaInternalMessage incoming, int comm)
    ...
    for (int i = 0, max1 = hashes.size(), max2 = inhashes.size(); i < max1 &&
        i < max2; i++) {
      byte[] hash = (byte[]) hashes.get(i);
      byte[] inhash = (byte[]) inhashes.get(i);
      if (!Arrays.equals(inhash, hash)) {
        KademliaInternalMessage req = new StoreRequestMessage(local, current,
            previous);
        server.send(req, origin.getAddress(), null);
      }
      previous = current;
      current -= interval;
      interval *= 2;
    ...
    if (inhashes.size() > hashes.size()) {
      KademliaInternalMessage req = new StoreRequestMessage(local,
          Long.MIN_VALUE, previous);
      server.send(req, origin.getAddress(), null);
    }
