...
public class TrivialQueryTestServer extends QueryTestServer {
  ...
  public TrivialQueryTestServer() {
    ...
    String queryConst = " FORWARD"
        ...
        + "  LET isConst = peerAge / peerAge "
        + "  LET isConstNext = peerAge - peerAge "
        + " REDUCE"
        + "  SUM(APPEND(peerAge,DQL_RESULTS))";

    String query = " FORWARD"
        + " TO"
        + "  LOAD(\"peerData.xml\", \"/peerData/friends/friend\", TRUE)"
        + " GATHER"
        + "  LET peerAge = IF(LEAF_EXECUTION, LOAD_NOISE(\"peerData.xml\" , \"/peerData/age\", FALSE),  LOAD (\"peerData.xml\" , \"/peerData/age\", FALSE))  "
    ...
    String query2 = "FORWARD" + " TO"
        + "  LOAD(\"peerData.xml\", \"/peerData/friends/friend\", TRUE) "
        + "GATHER " + " LET nextHop = 1" + " REDUCE "
        + "LENGTH(DQL_RESULTS) + 1";

    String query3 = " FORWARD"
        + " TO"
        + "  LOAD(\"peerData.xml\", \"/peerData/friends/friend\", TRUE) "
        + "   GATHER"
    ...
    tests1.put(0, new QueryDescription(queryConst, 3));
    tests1.put(1, new QueryDescription(query, 3));
    tests1.put(2, new QueryDescription(query2, 3));
    tests1.put(3, new QueryDescription(query3, 3));
    tests1.put(4, new QueryDescription(query4, 3));

    Map<Integer, QueryDescription> tests2 = new HashMap<Integer, QueryDescription>();
    tests1.put(0, new QueryDescription(queryConst, 3));
    tests2.put(1, new QueryDescription(query, 3));
    tests2.put(2, new QueryDescription(query2, 3));
    tests2.put(3, new QueryDescription(query3, 3));
    tests2.put(4, new QueryDescription(query4, 3));
