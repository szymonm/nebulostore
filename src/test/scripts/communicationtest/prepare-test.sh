...
ssh -l $USER $BOOTSTRAP_SERVER "ulimit -u 2000;\ cd $REMOTE_DIR/resources/conf/communication; cp CommunicationPeerServer.xml CommunicationPeer.xml"
...
for HOST in $SLICE_HOSTS; do 
    ...
    RDV=`expr $HOST_NO % 1`
    bash scp.sh $HOST $BUILD_DIR $REMOTE_DIR && ssh -l $USER $HOST "ulimit -u 2000; cd $REMOTE_DIR/resources/conf/communication"
    ((HOST_NO++))
done
